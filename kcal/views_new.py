...
def make_day_set(events, **kwargs):
    # Make day dict
    days = {}
    ...
    return {'month_data':month_data, 'days':new_days, 'good':good,}

#@cache_page(60 * 5)
def year(request, year):
    events = Event.objects.filter(time__year=year).order_by('time').select_related()
    return render_to_response('year.html',{'events':events})    
...
def month(request, year, month):
    ...
    return render_to_response('month.html', make_day_set(events,'month_val':month_val, 'max_cals':settings.TOTAL_ALLOWED_CALORIES,))
